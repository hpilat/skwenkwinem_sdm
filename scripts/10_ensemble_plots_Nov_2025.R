...
bioclim30s_ensemble_metrics_AUC <- bioclim30s_ensemble_metrics %>% 
  ...
  dplyr::rename(algorithm = wflow_id) %>% 
  # add model column and input "informed" in the rows
  add_column(model = "WorldClim", .before = "algorithm")

# now bind the rows together into 1 object:
ensemble_AUC <- rbind(informed_ensemble_metrics_AUC, bioclim30s_ensemble_metrics_AUC)

# write to new csv to import into word
write.csv(ensemble_AUC, file = "outputs/skwenkwinem_ensemble_metrics_AUC.csv")

ensemble_AUC <- read.csv("outputs/skwenkwinem_ensemble_metrics_AUC.csv", header = TRUE)
ensemble_AUC

# plot ensemble metrics together
# plot ensemble metrics together
ensemble_metrics_AUC <- ggplot(ensemble_AUC, aes(x = algorithm, y = mean, colour = model)) +
  geom_point(size = 1) +
...
# save to file
ggsave("outputs/ensemble_metrics_AUC.png", plot = ensemble_metrics_AUC, height = 6, width = 4)




# Isolate TSS metrics from model metrics:



# select only relevant columns and filter out all rows except roc_auc
informed_model_metrics_TSS <- informed_model_metrics %>% 
  dplyr::select("wflow_id", ".metric", "mean", "std_err") %>% 
  dplyr::filter(.metric == "tss_max")

bioclim30s_model_metrics_TSS <- bioclim30s_model_metrics %>% 
  dplyr::select("wflow_id", ".metric", "mean", "std_err") %>% 
  dplyr::filter(.metric == "tss_max")

# write to new csv to import into word
write.csv(informed_model_metrics_TSS, file = "outputs/skwenkwinem_informed_model_metrics_TSS.csv")
write.csv(bioclim30s_model_metrics_TSS, file = "outputs/skwenkwinem_bioclim30s_model_metrics_TSS.csv")

# select only relevant columns and filter out all rows except roc_auc
informed_ensemble_metrics_TSS <- informed_ensemble_metrics %>% 
  dplyr::select("wflow_id", ".metric", "mean", "std_err") %>% 
  dplyr::filter(.metric == "tss_max") %>% 
  # drop .metric column
  dplyr::select("wflow_id", "mean", "std_err") %>% 
...
bioclim30s_ensemble_metrics_TSS <- bioclim30s_ensemble_metrics %>% 
  dplyr::select("wflow_id", ".metric", "mean", "std_err") %>% 
  dplyr::filter(.metric == "tss_max") %>% 
  # drop .metric column
  dplyr::select("wflow_id", "mean", "std_err") %>% 
...
# now bind the rows together into 1 object:
ensemble_TSS <- rbind(informed_ensemble_metrics_TSS, bioclim30s_ensemble_metrics_TSS)

# write to new csv to import into word
write.csv(ensemble_TSS, file = "outputs/skwenkwinem_ensemble_metrics_TSS.csv")

ensemble_TSS <- read.csv("outputs/skwenkwinem_ensemble_metrics_TSS.csv", header = TRUE)
ensemble_TSS

# plot ensemble metrics together
ensemble_metrics_TSS <- ggplot(ensemble_TSS, aes(x = algorithm, y = mean, colour = model)) +
  geom_point(size = 1) +
  geom_errorbar(aes(ymin = mean - std_err, ymax = mean + std_err), width= 0.15, linewidth = 0.75) +
  # geom_hline(yintercept = 0.8, linetype = "dashed") +
  labs(y = "Mean TSS", x = "Algorithm") +
  theme_classic() +
  scale_x_discrete(labels = c("MaxEnt", "rf")) +
  scale_y_continuous(limits = c(0,1), 
                     expand = c(0,0)) +
...
ensemble_metrics_TSS

# save to file
ggsave("outputs/ensemble_metrics_TSS.png", plot = ensemble_metrics_TSS, height = 6, width = 4)
